  ...
  const addHandler = () => {
    ...
    if (location.state !== null) {
      axios.put("http://localhost:3008/adeditdnr/"+location.state.val._id,inputs)
        ...
        axios.post("http://localhost:3008/dnr", inputs)
        .then((res) => {
          console.log(res);
          // alert(res.data.message);
          Navigate('/adviewdnr')
...
<div  style={{paddingTop:"50px",marginLeft:"10px"}}>
    ...
    <TextField onChange={inputHandler} value={inputs.demail} placeholder='age' id="outlined-basic" name='demail' label="Age" variant="outlined" /><br/><br/>
    <TextField onChange={inputHandler} value={inputs.dage} placeholder='enter your email' id="outlined-basic" name='dage' label="Email" variant="outlined" /><br/><br/>
